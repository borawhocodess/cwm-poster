% Gemini theme
% https://github.com/anishathalye/gemini

\documentclass[final]{beamer}

% ====================
% Packages
% ====================

\usepackage[T1]{fontenc}
\usepackage{lmodern}
% \usepackage[size=custom,width=118.9,height=84.1,scale=1.0]{beamerposter}
\usepackage[size=custom,width=84.1,height=118.9,scale=1.0]{beamerposter}


% \makeatletter
% \def\input@path{{colorthemes/}}
% \makeatother

\usetheme{gemini}
\usecolortheme{itu}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{tikz}
\usepackage{pgfplots}
\pgfplotsset{compat=1.14}
\usepackage{anyfontsize}

% ====================
% Lengths
% ====================

% If you have N columns, choose \sepwidth and \colwidth such that
% (N+1)*\sepwidth + N*\colwidth = \paperwidth
\newlength{\sepwidth}
\newlength{\colwidth}
\setlength{\sepwidth}{0.01\paperwidth}
\setlength{\colwidth}{0.45\paperwidth}

\newcommand{\separatorcolumn}{\begin{column}{\sepwidth}\end{column}}

% ====================
% Title
% ====================

\title{
% \hspace*{\sepwidth}\RaggedRight
Causal-oriented representation learning for time-series forecasting \\
% \newline\hspace*{\sepwidth}
based on the spatiotemporal information transformation
}

\author{Sihua Cai \inst{1,3} \and Hao Peng \inst{1,2,3} \and Rui Liu \inst{1} \and Pei Chen \inst{1}}

\institute[shortinst]{\inst{1} School of Mathematics, South China University of Technology \samelineand \inst{2} School of Future Technology, South China University of Technology \samelineand \inst{3} These authors contributed equally.}

% ====================
% Footer (optional)
% ====================

\footercontent{
  Causal World Models Seminar Poster Day 2026
  \hfill
  Salih \underline{Bora} Öztürk - \href{mailto:oeztuers@cs.uni-freiburg.de}{oeztuers@cs.uni-freiburg.de}
  \hfill
  \href{https://github.com/borawhocodess/cwm-poster.git}{github.com/borawhocodess/cwm-poster}
}
% (can be left out to remove footer)

% ====================
% Logo (optional)
% ====================

% use this to include logos on the left and/or right side of the header:
% \logoright{\includegraphics[height=2.22cm]{img/logoufr.png}}
% \logoleft{\includegraphics[height=7cm]{logo2.pdf}}

% ====================
% Body
% ====================

\begin{document}

\begin{frame}[t]
\begin{columns}[t]
\separatorcolumn

\begin{column}{0.44\paperwidth}

  \begin{alertblock}{TL;DR}

    \textbf{CReP}: Causal-oriented Representation Learning Predictor

    is a self-supervised framework that jointly performs multi-step time-series forecasting and causal analysis by decomposing high-dimensional observations into orthogonal cause, effect, and non-causal representations for a chosen target variable.

  \end{alertblock}

  \begin{block}{Introduction}

    \vspace{-0.5em}\heading{Objectives}\vspace{-0.5em}

      \begin{itemize}
        \item From high-dimensional time-series observations:
        \begin{itemize}
          \item \textbf{Predict} the future trajectory of a selected target variable over multiple time steps.
          \item \textbf{Identify} which observed variables are likely \textbf{drivers} of the target and which are likely \textbf{responses}.
        \end{itemize}
      \end{itemize}

    \vspace{-0.5em}\heading{Problem}\vspace{-0.5em}

      \begin{itemize}
        \item Existing approaches forecast well but remain non-causal, or infer causality separately.
      \end{itemize}

    \vspace{-0.5em}\heading{Importance of the paper}\vspace{-0.5em}

      \begin{itemize}
        \item Enables \textbf{simultaneous forecasting and causal analysis} in high-dimensional time-series systems.
        \item Provides \textbf{interpretable causal insights} (causes vs.\ effects) rather than black-box predictions.
      \end{itemize}

    \vspace{-0.5em}\heading{Grounding}\vspace{-0.5em}

      \begin{itemize}

        \item \textbf{Dynamic causation}

        \begin{itemize}
          \item If variable $a$ drives $b$ in a dynamical system, then information about $a$ is embedded in the time series of $b$.
          \item Using delay embeddings, there exists an implicit mapping (Takens):
          \[
            A_t = (a_t, \dots, a_{t+L-1})^\top, \quad
            B_t = (b_t, \dots, b_{t+L})^\top, \quad
            A_t \approx h(B_t)
          \]
        \end{itemize}

        \item \textbf{Spatiotemporal information transformation (delay $\leftrightarrow$ non-delay)}

        \begin{itemize}
          \item STI transformation allows a learned non-delay embedding $C_t$ to be topologically conjugate to $B_t$:
          \(
            A_t \approx \tilde{h}(C_t)
          \)
          \item This motivates learning a latent $Z_t$ from which $Y_t$ can be predicted:
          \(
            Y_t \approx g(Z_t)
          \)
        \end{itemize}

      \end{itemize}

  \end{block}

  \begin{block}{Method}

    The self-supervised framework is characterized by three key features:
    \begin{enumerate}
      \item Dynamic causation detection with the STI transformation mechanism.
      \item Causal-oriented representation learning for multi-step predictions through the CReP.
      \item Causal analysis of the target variable via $\alpha\beta$-LRP.
    \end{enumerate}

    \begin{figure}
      \centering
      \includegraphics[width=0.93\linewidth]{img/crepfig1.png}
      \caption{Schematic illustration of the CReP framework.}
      \label{fig:crepfig1}
    \end{figure}

    \vspace{-0.5em}\heading{Loss}\vspace{-0.5em}

    \[
      \mathcal{L}=\lambda_1\mathcal{L}_{DS}+\lambda_2\mathcal{L}_{FC}+\lambda_3\mathcal{L}_{REC}+\lambda_4\mathcal{L}_{ORTH}
    \]

    \begin{itemize}
      \item \textbf{Determined-State Loss}: RMSE on known historical $y$.
      \item \textbf{Future-Consistency Loss}: RMSE between overlapping future estimates.
      \item \textbf{Reconstruction Loss}: assesses information recovery
        \begin{itemize}
          \item $\mathcal{L}_{REC\_X}$: spatiotemporal information $X$ from $(S,Z,V)$
          \item $\mathcal{L}_{REC\_S}$: latent cause representation $S$ recovered by $Y$
        \end{itemize}
      \item \textbf{Orthogonality Loss}: enforces orthogonality among $S$, $Z$, and $V$.
    \end{itemize}

  \end{block}

\end{column}

\separatorcolumn

\begin{column}{0.51\paperwidth}

  \begin{figure}
    \centering
    \includegraphics[width=0.80\linewidth]{img/crepsupfig1.png}
    \caption{The flowchart of CReP method.}
  \end{figure}

  \begin{block}{Results}

    \vspace{-0.5em}\heading{Simulation systems}\vspace{-0.5em}

    \begin{figure}
      \centering
      \begin{minipage}[t]{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{img/crepfig2forecast.png}
      \end{minipage}\hfill
      \begin{minipage}[t]{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{img/crepfig2causal.png}
      \end{minipage}\hfill
      \caption{CReP on Lorenz 96 (50-step history, 15-step-ahead prediction, 60D system).}
      \label{fig:crepfig2lorenz}
    \end{figure}

    \begin{table}
      \centering
      \small
      \begin{tabular}{l r r r r r r}
        \toprule
        & \multicolumn{2}{c}{Lorenz 96} & \multicolumn{2}{c}{Power grid} & \multicolumn{2}{c}{Dream4} \\
        \cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
        \textbf{Method}
        & \textbf{RMSE $\downarrow$} & \textbf{PCC $\uparrow$}
        & \textbf{RMSE $\downarrow$} & \textbf{PCC $\uparrow$}
        & \textbf{RMSE $\downarrow$} & \textbf{PCC $\uparrow$} \\
        \midrule
        CReP     & \textbf{0.1086} & \textbf{0.9908} & \textbf{0.1127} & \textbf{0.9868} & \textbf{0.0984} & 0.8894 \\
        ARNN     & 0.2328          & 0.8206          & 0.1501          & 0.9784          & 0.2281          & 0.8167 \\
        STICM    & 0.1822          & 0.9752          & 0.1530          & 0.9854          & 0.1080          & \textbf{0.9928} \\
        LSTM     & 0.4883          & 0.8303          & 0.3916          & 0.9659          & 0.8063          & 0.0615 \\
        ARIMA    & 0.4204          & 0.6156          & 0.2558          & 0.9583          & 0.2650          & 0.1218 \\
        SVR      & 0.8311          & 0.1618          & 0.4856          & 0.9540          & 0.6779          & 0.0566 \\
        Informer & 0.1543          & 0.9005          & 0.1163          & 0.9837          & 0.4716          & 0.0489 \\
        \bottomrule
      \end{tabular}
      \caption{Comparison to other forecasting methods.}
      \label{tab:crepsuptab4}
    \end{table}

    % TODO: find out how did they measure this strength

    \begin{figure}
      \centering
      \includegraphics[width=0.76\linewidth]{img/crepsupfig4dream.png}
      \caption{The normalized causal results of CReP on Dream4. Blue bars denote the top 10 variables by causal strength, purple bars highlight true causes or effects among them, and red bars indicate true causes or effects that were missed.}
      \label{fig:crepsupfig4dream}
    \end{figure}

    \vspace{-0.5em}\heading{Real datasets}\vspace{-0.5em}

    \begin{figure}
      \centering
      \begin{minipage}[t]{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{img/crepfig5hkforecast.png}
      \end{minipage}\hfill
      \begin{minipage}[t]{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{img/crepfig5hkcausal.png}
      \end{minipage}\hfill
      \caption{CReP on Hong Kong cardiovascular inpatients (70-day history, 25-day-ahead forecasting, 14 variables).}
      \label{fig:crepfig5hk}
    \end{figure}

  \end{block}

  \begin{block}{Conclusion}

    \vspace{-0.5em}\heading{Key Points}\vspace{-0.5em}
    \begin{itemize}
      \item Causes leave \textbf{detectable traces} in the dynamics of what they influence.
      \item CReP learns to \textbf{extract these traces} from high-dimensional time series.
      \item These representations are used to \textbf{predict the future} and \textbf{identify causes and effects}.
    \end{itemize}

    \vspace{-0.5em}\heading{Critical discussion}\vspace{-0.5em}
    \begin{itemize}
      \item Presence of \textbf{false positives} and \textbf{missed true causes} in Fig.~\ref{fig:crepsupfig4dream}.
      \item Table~\ref{tab:crepsuptab4} is limited to relatively \textbf{basic baselines} and does not include SOTA methods.
      \item Use of different hyperparameter choices across datasets.
    \end{itemize}

    \vspace{-0.5em}\heading{Future Work}\vspace{-0.5em}
    \begin{itemize}
      \item Integrating different causal learning methods to enhance applicability across diverse domains.
      \item Exploring causal detection methods through active intervention rather than passive observations.
    \end{itemize}

  \end{block}

  \begin{block}{References}

    \nocite{*}
    \footnotesize{\bibliographystyle{plain}\bibliography{poster}}

  \end{block}

\end{column}

\separatorcolumn
\end{columns}
\end{frame}

\end{document}
